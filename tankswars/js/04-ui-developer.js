    function getBattleModeSchedule(mode) {
      if (victoryDayEvent.modeIds.includes(mode.id)) {
        return {
          ...
          from: victoryDayEvent.fromDay,
          to: victoryDayEvent.toDay,
          label: "7-15 мая | ивент"
        };
      }
    ...
    function renderBattleModeScreen() {
      ...
      battleModes.forEach((mode) => {
        ...
        meta.textContent = `${mode.size} \u043d\u0430 ${mode.size}${schedule ? ` | ${schedule.label}` : ""}`;
