    ...
    function recordBattlePassProgress(result, stats) {
      ...
      const task = state.tasks.find((item) => item.day === today);
    ...
    function getAvailableBranchNations() {
      return [...new Set(loadedTanks.filter((tank) => !tank.futureTank && tank.techTreeEligible && tank.nation).map((tank) => tank.nation))]
    ...
    function claimBattlePassFinalReward() {
      ...
      const nation = state.branchNation || getAvailableBranchNations()[0] || "";

      if (!complete || state.finalClaimed || !nation) {
        return false;
      }

      loadedTanks
        .filter((tank) => !tank.futureTank && tank.techTreeEligible && tank.nation === nation)
        .forEach((tank) => {
          tank.state = 2;
          saveTankState(tank);
        });
      state.finalClaimed = true;
      state.branchNation = nation;
      saveBattlePassState(state);
      refreshSelectedTank();
      renderTopBar();
      renderTankBar(loadedTanks);
      renderEventsScreen();
      showGameNotification(`Ветка ${nation} полностью открыта`, "success");
      return true;
    }
    ...
    function createBattlePassPanel() {
      ...
      text.textContent = `31 ежедневное задание. Выполнено и забрано: ${completeCount} / 31. Финал: выбранная ветка техники полностью открывается.`;
      getAvailableBranchNations().forEach((nation) => {
      ...
      select.value = state.branchNation || select.value;
      select.addEventListener("change", () => {
        state.branchNation = select.value;
        saveBattlePassState(state);
      });
      state.tasks.forEach((task) => {
        const item = document.createElement("div");
        const claimButton = document.createElement("button");
        const locked = task.day > today;
        ...
        item.append(
          ...
          Object.assign(document.createElement("div"), {
            ...
            textContent: `${formatStoredNumber(task.progress)} / ${formatStoredNumber(task.target)}${locked ? " | откроется позже" : ""}`
          }),
          claimButton
        );
        grid.append(item);
