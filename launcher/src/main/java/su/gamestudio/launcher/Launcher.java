...
public final class Launcher {
    ...
    private String selectedGameId = "ashes-of-nations";
    ...
    private JComponent createUi() {
        ...
        Game selected = selectedGame(); String selectedTitle = selected == null ? "ASHES OF NATIONS" : selected.title.toUpperCase();
        side.add(label("GAMES STUDIO", 19, TEXT)); side.add(Box.createVerticalStrut(56)); JLabel mark = label(selectedTitle, 28, TEXT); side.add(mark); side.add(Box.createVerticalStrut(22)); styleButton(selectedAction, true); selectedAction.setMaximumSize(new Dimension(248, 54)); selectedAction.addActionListener(e -> runSelectedGame()); side.add(selectedAction); side.add(Box.createVerticalStrut(12)); JLabel installedLabel = small("●  ПРОВЕРКА ВЕРСИИ ИГРЫ"); side.add(installedLabel); side.add(Box.createVerticalStrut(28)); side.add(small("ОБ ИГРЕ")); JLabel info = new JLabel("<html><div style='width:210px'>" + (selected == null ? "Глобальная стратегия о странах, сценариях и управлении государством." : selected.description) + "<br><br><b>ЖАНР</b> &nbsp; Стратегия<br><b>ПЛАТФОРМЫ</b> &nbsp; ПК<br><b>СТУДИЯ</b> &nbsp; Games Studio<br><b>СТАТУС</b> &nbsp; Доступно</div></html>"); info.setForeground(MUTED); info.setFont(new Font("Dialog", Font.PLAIN, 11)); info.setAlignmentX(Component.LEFT_ALIGNMENT); side.add(Box.createVerticalStrut(8)); side.add(info); side.add(Box.createVerticalGlue()); side.add(nav("⇩  ОБНОВЛЕНИЯ", false, this::checkUpdates)); root.add(side, BorderLayout.WEST);
        ...
        JPanel tabs = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0)); tabs.setMaximumSize(new Dimension(Integer.MAX_VALUE, 54)); tabs.setBackground(Color.decode("#0a0e0b")); tabs.add(tab("◆", "TANKS WARS", "tanks-wars")); tabs.add(tab("✦", "ASHES OF NATIONS", "ashes-of-nations")); tabs.add(tab("◈", "SAVAGE ZONE", "savage-zone")); content.add(tabs); content.add(Box.createVerticalStrut(20));
        String artwork = selected == null ? "ashes-of-nations" : selected.id; JPanel banner = new JPanel(new BorderLayout()); banner.setBackground(Color.decode("#121b14")); banner.setMaximumSize(new Dimension(Integer.MAX_VALUE, 420)); banner.setPreferredSize(new Dimension(0, 420)); JLabel image = new JLabel(new ImageIcon(resourceImage(artwork, 920, 420))); banner.add(image, BorderLayout.CENTER); content.add(banner); content.add(Box.createVerticalStrut(14)); JLabel headline = label(selectedTitle + " — ЦЕНТР ИГРЫ", 27, TEXT); content.add(headline); content.add(Box.createVerticalStrut(7)); status.setForeground(MUTED); status.setFont(new Font("Dialog", Font.PLAIN, 12)); content.add(status); content.add(Box.createVerticalStrut(28)); content.add(label("НОВОСТИ И ИГРЫ", 22, TEXT)); content.add(Box.createVerticalStrut(12)); gamesPanel.setBackground(Color.decode("#061009")); gamesPanel.setAlignmentX(Component.LEFT_ALIGNMENT); content.add(gamesPanel);
    ...
    private JComponent tab(String icon, String text, String id) { boolean active = id.equals(selectedGameId); JButton tab = new JButton(icon + "   " + text); tab.setOpaque(true); tab.setBackground(active ? Color.decode("#1a291c") : Color.decode("#070b08")); tab.setForeground(active ? TEXT : Color.decode("#7f8f82")); tab.setBorder(new EmptyBorder(0, 20, 0, 20)); tab.setFocusPainted(false); tab.setFont(new Font("Dialog", Font.BOLD, 12)); tab.setPreferredSize(new Dimension(text.length() > 12 ? 198 : 145, 58)); tab.addActionListener(e -> selectGame(id)); return tab; }
    ...
    private void checkUpdates() {
        mainAction.setEnabled(false); status.setText("Проверяем обновления…"); new SwingWorker<Manifest, Void>() {
            ...
            protected void done() { try { manifest = get(); applyManifest("Все данные каталога актуальны"); boolean newer = isNewer(manifest.launcherVersion, APP_VERSION); updateDot.setVisible(newer); if (newer) status.setText("Доступно обновление лаунчера"); if (newer && !manifest.launcherUrl.isBlank()) askLauncherUpdate(); } catch (Exception e) { manifest = Manifest.fallback(); applyManifest("Сеть недоступна — открыт локальный каталог."); } finally { mainAction.setEnabled(true); } }
        }.execute();
    }
    ...
    private void runSelectedGame() {
        ...
        Game game = selectedGame(); if (game == null) return; if (game.url.isBlank()) { status.setText("Для " + game.title + " ещё не опубликован файл для ПК."); return; } String local = installed.getProperty(game.id + ".version", "");
    ...
    private Game selectedGame() { if (manifest == null) return Manifest.fallback().games.stream().filter(g -> g.id.equals(selectedGameId)).findFirst().orElse(null); return manifest.games.stream().filter(g -> g.id.equals(selectedGameId)).findFirst().orElse(manifest.games.isEmpty() ? null : manifest.games.get(0)); }
    private void selectGame(String id) { selectedGameId = id; window.setContentPane(createUi()); window.revalidate(); window.repaint(); if (manifest != null) applyManifest(status.getText()); }
    private void applyManifest(String message) { renderGames(); Game game = selectedGame(); String local = game == null ? "" : installed.getProperty(game.id + ".version", ""); selectedAction.setText(game == null ? "НЕДОСТУПНО" : game.url.isBlank() ? "СКАЧИВАНИЕ СКОРО" : local.isBlank() ? "СКАЧАТЬ" : isNewer(game.version, local) ? "ОБНОВИТЬ" : "ИГРАТЬ"); selectedAction.setEnabled(game != null); status.setText(message); }
    ...
    private record Manifest(String launcherVersion, String launcherUrl, List<Game> games) {
        ...
        static Manifest fallback() { return new Manifest("0.1.0", "", List.of(new Game("ashes-of-nations", "Ashes Of Nations", "Глобальная стратегия о странах, сценариях и управлении государством.", "0.1.0", "installer", "https://gamestudio.su/downloads/Ashes%20of%20Nations%20Setup%200.1.0.exe", ""), new Game("tanks-wars", "Tanks Wars", "Тактическая игра с ангаром, техникой и боями.", "1.1.1.1", "zip", "", "Tanks Wars.exe"))); }
    }
}
